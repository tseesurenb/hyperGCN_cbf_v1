...
def create_uuii_adjmat(df, verbose=-1):
    ...
    if u_sim == 'cos':
        ...
        cos_user_user_sim_matrix = sim.cosine_sim(user_item_matrix, top_k=u_top_k, self_loop = self_loop, verbose=verbose)
        jac_user_user_sim_matrix = sim.jaccard_sim(user_item_matrix, top_k=u_top_k, self_loop = self_loop, verbose=verbose)
        user_user_sim_matrix = (cos_user_user_sim_matrix + jac_user_user_sim_matrix)
    elif u_sim == 'pea': # Pearson similarity
        user_user_sim_matrix = sim.pearson_sim(user_item_matrix, top_k=u_top_k, self_loop = self_loop, verbose=verbose)
    else:
    ...
    if i_sim == 'cos':
        ...
        cos_item_item_sim_matrix = sim.cosine_sim(user_item_matrix.T, top_k=i_top_k, self_loop = config['self_loop'], verbose=verbose)
        jac_item_item_sim_matrix = sim.jaccard_sim(user_item_matrix.T, top_k=i_top_k, self_loop = config['self_loop'], verbose=verbose)
        item_item_sim_matrix = (cos_item_item_sim_matrix + jac_item_item_sim_matrix)
    elif i_sim == 'pea': # Pearson similarity'
        item_item_sim_matrix = sim.pearson_sim(user_item_matrix.T, top_k=i_top_k, self_loop = config['self_loop'], verbose=verbose)
    else:
        print(f'{br} The similarity metric {i_sim} is not supported yet !!!{rs}, available options: cos, jac, mix+')
